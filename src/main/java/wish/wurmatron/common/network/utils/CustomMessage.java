package wish.wurmatron.common.network.utils;
...
public abstract class CustomMessage <T extends CustomMessage <T>> implements IMessage,
  ...
  private static final <T extends CustomMessage <T>> void checkAndHandle (CustomMessage <T> msg,MessageContext ctx) {
    IThreadListener thread = ProjectWish.proxy.getThreadFromCTX (ctx);
    thread.addScheduledTask (() -> msg.process (ProjectWish.proxy.getPlayer (ctx),ctx.side));
  }
  ...
  public final IMessage onMessage (T msg, MessageContext ctx) {
    if (!msg.isValidOnSide (ctx.side))
      ...
      msg.process (ProjectWish.proxy.getPlayer (ctx),ctx.side);
    return null;
  }

...
public class ItemProspectPick extends ItemTool {
			...
			for (int x = -radius; x < radius; x++)
				for (int y = radiusFacing; y > 0; y--)
					for (int z = -radius; z < radius; z++) {
						...
						if (world.getBlockState (currentPos).getBlock () instanceof BlockOre && canSee (world.getBlockState (currentPos),player)) {
	...
	public boolean canSee (IBlockState state,EntityPlayer player) {
		if (Loader.isModLoaded ("orestages")) {
			if (OreTiersAPI.hasReplacement (state) && OreTiersAPI.hasStage (player,OreTiersAPI.getStageInfo (state).getFirst ()))
				return true;
			else

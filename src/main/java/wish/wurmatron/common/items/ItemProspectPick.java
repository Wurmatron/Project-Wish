...
public class ItemProspectPick extends ItemTool {
	...
	public ItemProspectPick (float attackDamageIn,float attackSpeedIn,ToolMaterial materialIn,Set<Block> effectiveBlocksIn) {
	...
	public void addInformation (ItemStack stack,@Nullable World world,List<String> tip,ITooltipFlag adv) {
		...
		tip.add (I18n.translateToLocal ("tooltip.mode.name").replaceAll ("%MODE%",mode.name ().substring (0,1) + mode.name ().substring (1,mode.name ().length ()).toLowerCase ()));
	}
	...
	public ActionResult<ItemStack> onItemRightClick (World world,EntityPlayer player,EnumHand hand) {
		if (player.isSneaking ()) {
			if (player.getHeldItem (hand).hasTagCompound ()) {
				player.getHeldItem (hand).getTagCompound ().setString ("mode",switchMode (getMode (player.getHeldItem (hand))).name ());
			} else {
				NBTTagCompound nbt = new NBTTagCompound ();
				nbt.setString ("mode",Type.NORMAL.name ());
			...
			if (player.world.isRemote)
				player.sendMessage (new TextComponentString (I18n.translateToLocal ("chat.mode.switch").replaceAll ("%MODE%",getMode (player.getHeldItem (hand)).name ())));
		}
	...
	public EnumActionResult onItemUse (EntityPlayer player,World world,BlockPos pos,EnumHand hand,EnumFacing facing,float x,float y,float z) {
		...
		HashMap<String, Integer> areaData = scanArea (player.getHeldItem (hand),world,pos,facing,player);
	...
	private String formatDisplay (String type,int amount) {
		if (amount >= 200)
			return I18n.translateToLocal ("chat.motherload.name").replaceAll ("%ORE%",type.replaceAll ("ore",""));
		else if (amount >= 150)
			return I18n.translateToLocal ("chat.huge.name").replaceAll ("%ORE%",type.replaceAll ("ore",""));
		else if (amount >= 100)
			return I18n.translateToLocal ("chat.large.name").replaceAll ("%ORE%",type.replaceAll ("ore",""));
		else if (amount >= 50)
			return I18n.translateToLocal ("chat.medium.name").replaceAll ("%ORE%",type.replaceAll ("ore",""));
		else if (amount >= 25)
			return I18n.translateToLocal ("chat.small.name").replaceAll ("%ORE%",type.replaceAll ("ore",""));
		else if (amount >= 10)
			return I18n.translateToLocal ("chat.traces.name").replaceAll ("%ORE%",type.replaceAll ("ore",""));
		return "Error finding ore!";
	}

	private HashMap <String, Integer> scanArea (ItemStack stack,World world,BlockPos pos,EnumFacing facing,EntityPlayer player) {
			...
			for (int x = -radius; x < radius; x++)
				for (int y = radiusFacing; y > 0; y--)
					for (int z = -radius; z < radius; z++) {
						...
						if (world.getBlockState (currentPos).getBlock () instanceof BlockWishOre && canSee (world.getBlockState (currentPos),player)) {
							Block block = world.getBlockState (currentPos).getBlock ();
							scanData.put (block.getLocalizedName (),scanData.getOrDefault (block.getLocalizedName (),0) + 1);
						}
					}
	...
	public boolean canSee (IBlockState state,EntityPlayer player) {
		if (Loader.isModLoaded ("orestages"))
			if (OreTiersAPI.getStageInfo (state) != null && GameStageHelper.getPlayerData (player).hasStage (OreTiersAPI.getStageInfo (state).getFirst ()))
				return true;
			else

...
public class GravelWish extends BlockSlideGravity {

  private int amount;

  public GravelWish(Material material, int amount) {
    super(material);
    this.amount = amount;
    setHardness(3f);
    setResistance(10f);
  ...
  public Item getItemDropped(IBlockState state, Random rand, int fortune) {
    if (state.getBlock().equals(WishBlocks.gravelIgneous)) {
      return Item.getItemFromBlock(WishBlocks.gravelIgneous);
    } else if (state.getBlock().equals(WishBlocks.gravelMetamorphic)) {
      return Item.getItemFromBlock(WishBlocks.gravelMetamorphic);
    } else {
      return Item.getItemFromBlock(WishBlocks.gravelSedimentary);
    }
  }

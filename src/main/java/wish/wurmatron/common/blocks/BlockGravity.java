...
public class BlockGravity extends BlockFalling {
		...
		private static BlockPos checkSide(World world, BlockPos pos, boolean downCheck) {
				...
				if (possibleLocations.size() > 0) {
						...
						if (!world.isAirBlock(location.up())) world.setBlockToAir(location.up());
						if (!world.isAirBlock(location)) world.setBlockToAir(location);
						return location;
				}
				return null;
		}
		...
		private static boolean isValidMove(World world, BlockPos pos) {
				if (world.isAirBlock(pos) || canFallThrough(world.getBlockState(pos))) return true;
				return false;
		}

...
public class DimTransferEvent {
  ...
  public static void teleportPlayerTo(EntityPlayerMP player, double x, double y, double z,
      int dimID) {
  ...
  public void onPlayerTick(TickEvent.PlayerTickEvent e) {
    if (e.side == Side.SERVER && e.phase == TickEvent.Phase.START
        && e.player.world.getWorldTime() % 10 == 0 && travelData.size() > 0 && e.player.posY <= 0) {
      if (travelData.containsKey(e.player.dimension)) {
        ...
        if (e.player.posY < data.yTravel) {
          ...
          teleportPlayerTo((EntityPlayerMP) e.player, ((EntityPlayerMP) e.player).posX,
              data.spawnHeight, ((EntityPlayerMP) e.player).posZ, data.dimensionID);
  ...
  public void onPlayerFall(LivingFallEvent e) {
    if (e.getEntity() instanceof EntityPlayer
        & e.getEntity().getEntityData().getInteger("Traveled") >= 0) {
      e.getEntity().attackEntityFrom(DamageSource.FALL,
          e.getEntity().getEntityData().getInteger("Traveled"));
